...
import com.model.*;
@WebServlet(name = "ShoppingCartServlet", urlPatterns = {"/addToCart","/showProduct", "/deleteItem", "/cleanItem"  })
public class ControllerServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        }else if (uri.endsWith("cleanItem")) {
        	cleanItem(request, response);
        }
    }
    ...
    // 清空购物车并减少商品库存
    private void cleanItem(HttpServletRequest request, HttpServletResponse response) 
                    throws IOException {
    	  HttpSession session = request.getSession();
          ShoppingCart cart = (ShoppingCart) session.getAttribute("cart"); 
    	     ...
    	     for(GoodsItem shopItem: cart.getItems()){
    	    	 ...
    	    	 for (Product product : products) {
        	          if(shopItem.getProduct().getId()==product.getId()){
        		         ...
        		         products.set(order, new Product(product.getId(), product.getPname(), product.getPrice(), product.getPnum()-num, product.getPtype(),product.getPicid()));                
        	          }
        	          order= order +1;
        	    }
             }
